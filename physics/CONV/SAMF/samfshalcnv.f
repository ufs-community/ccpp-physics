...
c  physical parameters
      ...
      parameter(cthk=200._conv_wp,cthkmn=0._conv_wp,dthk=25._conv_wp)
...
!  shevf is an enhancing evaporation factor for shallow convection
      parameter(cinacrmx=-120._conv_wp,shevf=2.0_conv_wp)
      parameter(dtmax=10800._conv_wp,dtmin=600._conv_wp)
      parameter(bb1=4.0_conv_wp,bb2=0.8_conv_wp,csmf=0.2_conv_wp)
      parameter(tkcrt=2._conv_wp,cmxfac=10._conv_wp)
             ...
             do i = 1, im
               if (cnvflg(i)) then
                 if(k > kb(i) .and. k < kmax(i)) then
                   ...
                   tem  = cq * tem
                   factor = 1._conv_wp + tem
                   ecko(i,k,kk) = ((1._conv_wp - tem) * ecko(i,k-1,kk) &
        ...
        do i = 1, im
          if (cnvflg(i)) then
            if(k > kb(i) .and. k < kbcon1(i)) then
              ...
              rfact =  1._conv_wp + real(fv, kind=conv_wp) * real(cp,
...
!> - Calculate the cloud top as the first level where parcel buoyancy becomes negative; the maximum possible value is at \f$p=0.7p_{sfc}\f$.
      ...
      do i=1,im
        if (flg(i) .and. k < kbm(i)) then
          if(k > kbcon1(i) .and. dbyo(i,k) < 0._conv_wp) then
             ktcon(i) = k
             flg(i)    = .false.
          endif
        endif
...
!
     ...
     &                    / (real(cp, kind=conv_wp) * to(i,k))) 
     &                    * dbyo(i,k) / (1._conv_wp + gamma) * rfact
                val = 0._conv_wp
                buo(i,k) = buo(i,k) + real(grav, kind=conv_wp) 
...
!
     ...
     &                     omegain_loc,real(delt, kind=conv_wp),del,zi,
     &                     cnvflg,omegaout_loc,real(grav, kind=conv_wp),
     &                     buo,drag,wush,xlamue,bb1,bb2)
...
!convert to Pa/s for use in closure
         do k = 2, km1
            do i = 1, im
               if (cnvflg(i)) then
                  if(k > kbcon1(i) .and. k < ktcon(i)) then
                     rho = po(i,k)*100._conv_wp/(real(rd, kind=conv_wp)
     &                    * to(i,k))
...
!> - For progsigma = T, calculate the xi term in Bengtsson et al. 2022 \cite Bengtsson_2022 (equation 8)
         do k = 2, km1
            do i = 1, im
               if (cnvflg(i)) then
                  if(k > kbcon1(i) .and. k < ktcon(i)) then
                     if(omega_u(i,k) .ne. 0._conv_wp)then
...
cj
     ...
     &    +  tem1*eta(i,k-1)*.5_conv_wp*(hcko(i,k)+hcko(i,k-1))*dz
     &         ) * factor
cj
      ...
      do k=1,km1
        do i=1,im
          if(cnvflg(i) .and. k <= ktcon(i)) then
            q_diff(i,k) = real(q1(i,k), kind=conv_wp) 
     &                  - real(q1(i,k+1), kind=conv_wp)
          endif
        enddo
      enddo
      do i=1,im
        if(cnvflg(i)) then
          if(real(q1(i,1), kind=conv_wp) >= 0._conv_wp) then
            q_diff(i,0) = max(0._conv_wp,2._conv_wp*real(q1(i,1), 
     &                    kind=conv_wp)-real(q1(i,2), kind=conv_wp))
     &                   -real(q1(i,1), kind=conv_wp)
          else
            q_diff(i,0) = min(0._conv_wp,2._conv_wp*real(q1(i,1), 
     &                    kind=conv_wp)-real(q1(i,2), kind=conv_wp))
     &                   -real(q1(i,1), kind=conv_wp)
          endif
        endif
        ...
        do i=1,im
          if(cnvflg(i)) then
            if(ctr(i,1,n) >= 0._conv_wp) then
              e_diff(i,0,n) = max(0._conv_wp,2._conv_wp*ctr(i,1,n)
     &                       -ctr(i,2,n))-ctr(i,1,n)
            else
              e_diff(i,0,n) = min(0._conv_wp,2._conv_wp*ctr(i,1,n)
     &                       -ctr(i,2,n))-ctr(i,1,n)
            endif
          endif
        enddo
...
!> - Following Bechtold et al. (2008) \cite bechtold_et_al_2008, calculate the convective turnover time using the mean updraft velocity (wc) and the cloud depth. It is also proportional to the grid size (gdx).
      do i= 1, im
        if(cnvflg(i)) then
          ...
          dtconv(i) = tem / wc(i)
          if (.not.hwrf_samfshal) then
            tfac = 1._conv_wp + gdx(i) / 75000._conv_wp
...
!      Initial computations, dynamic q-tendency
     ...
     &        real(hvap, kind=conv_wp),real(delt, kind=conv_wp),qadv,kb,
     &        kbcon1,ktcon,cnvflg,real(betascu, kind=conv_wp),
     &        real(betamcu, kind=conv_wp),real(betadcu, kind=conv_wp),
     &        sigmind,sigminm,sigmins,sigmain_loc,sigmaout_loc,sigmab)
...
!!  As discussed in Han et al. (2017) \cite han_et_al_2017 , when dtconv is larger than tauadv, the convective mixing is not fully conducted before the cumulus cloud is advected out of the grid cell. In this case, therefore, the cloud base mass flux is further reduced in proportion to the ratio of tauadv to dtconv.
      ...
      do i= 1, im
        if(cnvflg(i)) then
          k = kbcon(i)
          rho = po(i,k)*100._conv_wp / (real(rd, kind=conv_wp)*to(i,k))
...
!> - For scale-aware parameterization, the updraft fraction (sigmagfm) is first computed as a function of the lateral entrainment rate at cloud base (see Han et al.'s (2017) \cite han_et_al_2017 equation 4 and 5), following the study by Grell and Freitas (2014) \cite grell_and_freitas_2014.
      do i = 1, im
        if(cnvflg(i)) then
          tem = min(max(xlamue(i,kbcon(i)), 2.e-4_conv_wp), 6.e-4_conv_wp)
          tem = 0.2_conv_wp / tem
...
!> - Then, calculate the reduction factor (scaldfunc) of the vertical convective eddy transport of mass flux as a function of updraft fraction from the studies by Arakawa and Wu (2013) \cite arakawa_and_wu_2013 (also see Han et al.'s (2017) \cite han_et_al_2017 equation 1 and 2). The final cloud base mass flux with scale-aware parameterization is obtained from the mass flux when sigmagfm << 1, multiplied by the reduction factor (Han et al.'s (2017) \cite han_et_al_2017 equation 2).
      do i = 1, im
        if(cnvflg(i)) then
          if (gdx(i) < dxcrt) then
             if(progsigma)then
              scaldfunc(i) = (1._conv_wp-sigmab(i)) * (1._conv_wp
     &                      -sigmab(i))
             else
              scaldfunc(i) = (1._conv_wp-sigmagfm(i)) * (1._conv_wp
     &                      -sigmagfm(i))
             endif
...
!
     ...
     &              kind=conv_wp)
              if(real(q1(i,k), kind=conv_wp) < 0._conv_wp) tsumn(i) = 
     &        tsumn(i) + tem
              if(real(q1(i,k), kind=conv_wp) > 0._conv_wp) tsump(i) = 
     &        tsump(i) + tem
            endif
          endif
        enddo
          ...
          if (cnvflg(i)) then
            if(k > kb(i) .and. k <= ktcon(i)) then
              if(rtnp(i) < 0._conv_wp) then
                if(tsump(i) > abs(tsumn(i))) then
                  if(real(q1(i,k), kind=conv_wp) < 0._conv_wp) q1(i,k)=
     &               0._kind_phys
                  if(real(q1(i,k), kind=conv_wp) > 0._conv_wp) q1(i,k)= 
     &               real((1._conv_wp+rtnp(i)), kind=kind_phys)*q1(i,k)
                else
                  if(real(q1(i,k), kind=conv_wp) < 0._conv_wp) q1(i,k)= 
     &               real((1._conv_wp+rtnp(i)), kind=kind_phys)*q1(i,k)
                  if(real(q1(i,k), kind=conv_wp) > 0._conv_wp) q1(i,k)=
            ...
            if(k > kb(i) .and. k <= ktcon(i)) then
              if(rtnp(i) < 0._conv_wp) then
                if(tsump(i) > abs(tsumn(i))) then
                  ...
                  if(ctr(i,k,n)>0._conv_wp) ctr(i,k,n)=(1._conv_wp
     &              +rtnp(i))*ctr(i,k,n)
                else
                  if(ctr(i,k,n)<0._conv_wp) ctr(i,k,n)=(1._conv_wp
     &              +rtnp(i))*ctr(i,k,n)
          ...
          do k = 2, km1
            do i = 1, im
              if (cnvflg(i)) then
                if(k > kb(i) .and. k < ktcon(i)) then
                  dp = 1000._conv_wp * del(i,k)
                  if (real(qtr(i,k,kk), kind=conv_wp) < 0._conv_wp) then
      ...
      do k = km, 1, -1
        do i = 1, im
          if (k <= kmax(i)) then
            ...
            if(cnvflg(i)) then
              if(k < ktcon(i) .and. k > kb(i)) then
                rn(i) = rn(i) + pwo(i,k) * xmb(i) * .001_conv_wp * dt2
              endif
            endif
...
!              if(islimsk(i) == 1) evef=.07
     ...
     &                    kind=kind_phys)
                t1(i,k) = real(t1(i,k), kind=kind_phys) - real(elocp
...
cj
      do i = 1, im
        if(cnvflg(i)) then
          if(rn(i) < 0._conv_wp .or. .not.flg(i)) rn(i) = 0._conv_wp
          ktop(i) = ktcon(i)
          kbot(i) = kbcon(i)
          kcnv(i) = 2
        endif
...
!> - Calculate convective cloud cover, which is used when pdf-based cloud fraction is used (i.e., pdfcld=.true.).
      do k = 1, km
        do i = 1, im
          if (cnvflg(i)) then
            if (k >= kbcon(i) .and. k < ktcon(i)) then
              cnvc(i,k) = real(0.04_conv_wp * log(1._conv_wp
     &                  + 675._conv_wp * eta(i,k) * xmb(i)),
     &                    kind=kind_phys)
        ...
        do i = 1, im
          if(cnvflg(i)) then
            if(k > kb(i) .and. k < ktop(i)) then
              ...
              tem1 = pfld(i,k) * 100._conv_wp / (real(rd, kind=conv_wp)
