...
!  parameters for prognostic sigma closure                                                                                                                                                      
     ...
     &     tentr(im,km)
      ...
      parameter(sigmind=0.01_conv_wp,sigmins=0.03_conv_wp,
     &          sigminm=0.01_conv_wp)
...
!     parameter(fact1=(cvap-cliq)/rv,fact2=hvap/rv-fact1*t0c)
      parameter(clamd=0.03_conv_wp,tkemx=0.65_conv_wp,
     &          tkemn=0.05_conv_wp)
...
!     parameter(cinacrmx=-120.,cinacrmn=-120.)
      ...
      parameter(tkcrt=2.0_conv_wp,cmxfac=15.0_conv_wp)
...
!
      do i=1,im
        cnvflg(i) = .true.
        if(do_mynnedmf) then
            if(maxMF(i).gt.0.0_conv_wp)cnvflg(i)=.false.
        endif
        sfcpbl(i) = real(sfclfac, kind=conv_wp) * real(hpbl(i), 
     &              kind=conv_wp)
        rn(i)=0.0_conv_wp
        mbdt(i)=10.0_conv_wp
        kbot(i)=km+1
        ...
        ktcon(i)=1
        ktconn(i)=1
        dtconv(i) = 3600.0_conv_wp
        cldwrk(i) = 0.0_conv_wp
        pdot(i) = 0.0_conv_wp
        lmin(i) = 1
        jmin(i) = 1
...
!>  - Initialize updraft and downdraft mass fluxes to zero.
      do k = 1, km
        do i = 1, im
          ud_mf(i,k) = 0.0_conv_wp
          dd_mf(i,k) = 0.0_conv_wp
          dt_mf(i,k) = 0.0_conv_wp
        enddo
      enddo
      if(mp_phys == mp_phys_mg) then
        do k = 1, km
          do i = 1, im
            QLCN(i,k)      = real(qtr(i,k,2), conv_wp)
            QICN(i,k)      = real(qtr(i,k,1), conv_wp)
...
!           tem       = grav * zo(i,k) + cp * to(i,k)
            tem = real(phil(i,k), conv_wp) + real(cp, conv_wp) * 
     &                 to(i,k)
            heo(i,k) = tem + real(hvap, conv_wp) * qo(i,k)
            heso(i,k) = tem + real(hvap, conv_wp) * qeso(i,k)
...
!> - Calculate the temperature, specific humidity, and pressure at interface levels.
     ...
     &                real(fact2,conv_wp) / (to(i,k+1)**2))
            ...
            gamma   = real(el2orc, conv_wp) * qeso(i,k+1) /
     &                (to(i,k+1)**2)
...
!         pdot(i)  = 10.* dot(i,kbcon(i))
          pdot(i)  = 0.01_conv_wp * 
     &               real(dot(i,kbcon(i)), conv_wp) ! Now dot is in Pa/s
        endif
      enddo
c
        ...
        do k = 1, km1
          do i = 1, im
            if(cnvflg(i)) then
              if(k >= kb(i) .and. k < kbcon(i)) then
                dz = zo(i,k+1) - zo(i,k)
                tkemean(i) = tkemean(i) + real(tkeh(i,k), conv_wp) * dz
                sumx(i) = sumx(i) + dz
              endif
            endif
          enddo
        ...
        do i= 1, im
          if(cnvflg(i)) then
             ...
             if(tkemean(i) > tkemx) then
               clamt(i) = real(clam, conv_wp) + real(clamd, conv_wp)
...
!
        if(do_ca .and. ca_entr)then
            do i=1,im
               if(cnvflg(i)) then
                  if(real(ca_deep(i), conv_wp) > real(nthresh, 
     &               conv_wp))then
                     clamt(i) = real(clam, conv_wp) -
     &                          real(clamca, conv_wp)
                  else
                     clamt(i) = real(clam, conv_wp)
                  endif
        ...
        do i=1,im
          if(cnvflg(i)) then
            ...
            if(tkemean(i) > tkcrt) then
              ...
              tem1 = min(tem, cmxfac)
              clamt(i) = tem1 * real(clam, conv_wp)
...
!
        do i=1,im
          if(cnvflg(i)) then
            xlamdet(i) = real(xlamde, conv_wp)
            xlamddt(i) = real(xlamdd, conv_wp)
            cxlamet(i) = real(cxlame, conv_wp)
            cxlamdt(i) = real(cxlamd, conv_wp)
          endif
        enddo
!
      ...
      do k = 1, km1
        do i=1,im
          if(cnvflg(i)) then
            ...
            xlamue(i,k) = max(xlamue(i,k), real(crtlame, conv_wp))
          endif
        enddo
      enddo
c
...
c
     ...
     &      (k > kbcon(i) .and. k < kmax(i))) then
              ...
              fent1(i,k) = min(tem**2, 3.0_conv_wp)
              fent2(i,k) = min(tem**3, 5.2_conv_wp)
          endif
        enddo
      enddo
c
...
!> - Set cloud properties equal to the state variables at updraft starting level (kb).
      do i = 1, im
        if(cnvflg(i)) then
          indx         = kb(i)
          hcko(i,indx) = real(heo(i,indx), conv_wp)
          ucko(i,indx) = real(uo(i,indx), conv_wp)
          vcko(i,indx) = real(vo(i,indx), conv_wp)
          pwavo(i)     = 0.0_conv_wp
        endif
      enddo
      if (.not.hwrf_samfdeep) then
!  for tracers
       do n = 1, ntr
        do i = 1, im
          if(cnvflg(i)) then
            indx = kb(i)
            ecko(i,indx,n) = real(ctro(i,indx,n), conv_wp)
            ercko(i,indx,n) = real(ctro(i,indx,n), conv_wp)
          endif
        enddo
       enddo
      endif
...
!
              tem  = 0.5_conv_wp * real(cm,conv_wp) * tem
              factor = 1.0_conv_wp + tem
              ptem = tem + real(pgcon,conv_wp)
              ptem1= tem - real(pgcon,conv_wp)
              ucko(i,k)=  ((1.0_conv_wp-tem)*ucko(i,k-1)+ptem*
     &                    uo(i,k)+ptem1*uo(i,k-1))/factor
              vcko(i,k)=  ((1.0_conv_wp-tem)*vcko(i,k-1)+ptem*
     &                    vo(i,k)+ptem1*vo(i,k-1))/factor
            endif
          endif
        enddo
      enddo
        ...
        do i = 1, im
          if (cnvflg(i)) then
            if(k > kb(i) .and. k < kbcon1(i)) then
              ...
              rfact =  1.0_conv_wp + real(fv, conv_wp) *
     &                 real(cp, conv_wp) *gamma *to(i,k) /
     &                 real(hvap, conv_wp)
              cina(i) = cina(i) +
!    &                 dz1 * eta(i,k) * (grav / (cp * to(i,k)))
     &                 dz1 * (real(grav, conv_wp) / (real(cp, conv_wp) *
     &                 to(i,k))) * dbyo(i,k) / (1.0_conv_wp + gamma) *
     &                 rfact
              val = 0.0_conv_wp
              cina(i) = cina(i) +
!    &                 dz1 * eta(i,k) * grav * fv *
     &                 dz1 * real(grav, conv_wp) * real(fv, conv_wp) *
      ...
      if(hwrf_samfdeep) then
       ...
       do i = 1, im
        if(cnvflg(i)) then
          ...
          tem = min(tem,val2)
          tem = 1.0_conv_wp - tem
          tem = 0.5_conv_wp*(real(cinacrmx, conv_wp) - 
     &          real(cinacrmn, conv_wp))
          cinacr = real(cinacrmx, conv_wp) - tem * tem1
          if(cina(i) < cinacr) cnvflg(i) = .false.
        endif
       enddo
      endif !hwrf_samfdeep
...
c
      do i = 1, im
        if(cnvflg(i)) then
          ...
          if(tem < real(cthk, conv_wp)) cnvflg(i) = .false.
        endif
      enddo
...
!> - Calculate the maximum value of the cloud base mass flux using the CFL-criterion-based formula of Han and Pan (2011) \cite han_and_pan_2011, equation 7.
      do i = 1, im
        if(cnvflg(i)) then
          ...
          xmbmax(i) = dp / (real(grav, conv_wp) * real(dt2, conv_wp))
        endif
      enddo
c
        ...
        do i = 1, im
          if (cnvflg(i)) then
            if(k > kb(i) .and. k < ktcon(i)) then
              dz    = zi(i,k) - zi(i,k-1)
              gamma = real(el2orc, conv_wp) * qeso(i,k) / (to(i,k)**2)
              qrch = qeso(i,k) + gamma * dbyo(i,k) / (real(hvap,
     &               conv_wp) * (1.0_conv_wp + gamma))
cj
              tem  = 0.25_conv_wp * (xlamue(i,k)+xlamue(i,k-1)) * dz
              tem  = real(cq, conv_wp) * tem
              factor = 1.0_conv_wp + tem
...
c
              if(k >= kbcon(i) .and. dq > 0.0_conv_wp) then
                ...
                if(ncloud > 0 .and. k > jmin(i)) then
                  ptem = c0t(i,k) + real(c1, conv_wp)
                  qlk = dq / (eta(i,k) + etah * ptem * dz)
                  dellal(i,k) = etah * real(c1, conv_wp) * dz * qlk *
...
!               aa1(i) = aa1(i) - dz * grav * qlk
                buo(i,k) = buo(i,k) - real(grav, conv_wp) * qlk
                qcko(i,k) = qlk + qrch
                pwo(i,k) = etah * c0t(i,k) * dz * qlk
                pwavo(i) = pwavo(i) + pwo(i,k)
!               cnvwt(i,k) = (etah*qlk + pwo(i,k)) * grav / dp
                cnvwt(i,k) = etah * qlk * real(grav, conv_wp) / dp
...
!
     ...
     &                   real(cp, conv_wp)* gamma * to(i,k) / 
     &                   real(hvap, conv_wp)
                buo(i,k) = buo(i,k) + (real(grav, conv_wp) /
     &                     (real(cp, conv_wp) * to(i,k))) * dbyo(i,k) /
...
!NRL MNM: Limit overshooting not to be deeper than half the actual cloud              
              ...
              if(aa2(i) < 0.0_conv_wp .or. tem1 >= tem) then
                ktcon1(i) = k
                flg(i) = .false.
              endif
            endif
...
!> - if progomega = true, calculate prognostic updraft velocity (Pa/s) according to progomega routine.
      ...
      if (hwrf_samfdeep) then
         do i = 1, im
            if (cnvflg(i)) then
               k = kbcon1(i)
               tem = po(i,k) / (real(rd, conv_wp) * to(i,k))
               wucb = -0.01_conv_wp * real(dot(i,k), conv_wp) /
     &                (tem * real(grav, conv_wp))
               if(wucb.gt.0.0_conv_wp) then
                  wu2(i,k) = wucb * wucb
               else
                  wu2(i,k) = 0.0_conv_wp
               endif
         ...
         do k = 2, km1
            do i = 1, im
               if (cnvflg(i)) then
                  if(k > kbcon1(i) .and. k < ktcon(i)) then
                     dz    = zi(i,k) - zi(i,k-1)
                     tem  = 0.25_conv_wp * real(bb1, conv_wp) *
     &                      (drag(i,k-1)+drag(i,k)) * dz
                     tem1 = 0.5_conv_wp * real(bb2, conv_wp) *
     &                      (buo(i,k-1)+buo(i,k))
...
!! where \f$\Delta V\f$ is the integrated horizontal shear over the cloud depth, \f$\Delta z\f$, (the ratio is converted to units of \f$10^{-3} s^{-1}\f$). The variable "edto" is \f$1-E\f$ and is constrained to the range \f$[0,0.9]\f$.
      ...
      do k = 2, km
        do i = 1, im
          if (cnvflg(i)) then
            if(k > kb(i) .and. k <= ktcon(i)) then
              shear = sqrt((uo(i,k) - uo(i,k-1))**2 + (vo(i,k) - 
     &                vo(i,k-1))**2 )
      ...
      do i = 1, im
        if(cnvflg(i)) then
          vshear(i) = 1.e3_conv_wp * vshear(i) / (zi(i,ktcon(i))-
     &                zi(i,kb(i)))
          e1 = 1.591_conv_wp - .639_conv_wp * vshear(i) +
     &         .0953_conv_wp * (vshear(i)**2) - .00496_conv_wp *
     &         (vshear(i)**3)
          edt(i)=1.0_conv_wp-e1
          val =  .9_conv_wp
          edt(i) = min(edt(i),val)
          val = .0_conv_wp
          edt(i) = max(edt(i),val)
...
!! \f$\lambda_d\f$ is the detrainment rate, \f$\beta\f$ is a constant currently set to 0.05, implying that only 5% of downdraft mass flux at LFC reaches the ground surface due to detrainment, \f$k_{LFC}\f$ is the vertical index of the LFC level, and \f$\overline{\Delta z}\f$ is the average vertical grid spacing below the LFC.
      ...
      enddo
      do i = 1, im
        beta = real(betas, conv_wp)
        if(islimsk(i) == 1) beta = real(betal, conv_wp)
        if(cnvflg(i)) then
          dz  = (sumx(i)+zi(i,1))/real(kd94(i), conv_wp)
          tem = 1.0_conv_wp/real(kd94(i), conv_wp)
          xlamd(i) = (1.0_conv_wp-beta**tem)/dz
        endif
      enddo
c
        ...
        do i = 1, im
          if (cnvflg(i) .and. k <= kmax(i)-1) then
           if(k < jmin(i) .and. k >= kd94(i)) then
              ...
              etad(i,k) = etad(i,k+1) * (1.0_conv_wp - ptem * dz)
      ...
      do k = km1, 1, -1
        do i = 1, im
          if (cnvflg(i) .and. k < jmin(i)) then
              ...
              factor = 1.0_conv_wp + tem - tem1
              hcdo(i,k) = ((1.0_conv_wp-tem1)*hcdo(i,k+1)+tem*
     &                    0.5_conv_wp*(heo(i,k)+heo(i,k+1)))/factor
...
!
              ...
              ptem = tem - real(pgcon, conv_wp)
              ptem1= tem + real(pgcon, conv_wp)
              ucdo(i,k)=  ((1.0_conv_wp-tem)*ucdo(i,k+1)+ptem*
     &                    uo(i,k+1)+ptem1*uo(i,k))/factor
              vcdo(i,k)=  ((1.0_conv_wp-tem)*vcdo(i,k+1)+ptem*
     &                    vo(i,k+1)+ptem1*vo(i,k))/factor
          endif
        enddo
      enddo
      if(.not.hwrf_samfdeep) then
      ...
      do k = km1, 1, -1
        do i = 1, im
          if (cnvflg(i) .and. k < jmin(i)) then
              ...
              tem  = real(cq, conv_wp) * tem
              factor = 1.0_conv_wp + tem
              ecdo(i,k,n) = ((1.0_conv_wp-tem)*ecdo(i,k+1,n)+tem*
     &                     (ctro(i,k,n)+ctro(i,k+1,n)))/factor
          endif
        enddo
      enddo
...
!> - Compute the amount of moisture that is necessary to keep the downdraft saturated.
      do k = km1, 1, -1
        do i = 1, im
          if (cnvflg(i) .and. k < jmin(i)) then
              gamma      = real(el2orc, conv_wp) * qeso(i,k) / 
     &                     (to(i,k)**2)
...
!             aa1(i)=aa1(i)+edto(i)*dz*etad(i,k)
     ...
     &             conv_wp))
              val = 0.0_conv_wp
!             aa1(i)=aa1(i)+edto(i)*dz*etad(i,k)
              aa1(i) = aa1(i)+edto(i)*dz*real(grav, conv_wp)*
     &                 real(fv, conv_wp)*max(val,(qeso(i,k)-qo(i,k)))
          endif
        enddo
      enddo
!> - Check for negative total cloud work function; if found, return to calling routine without modifying state variables.
      do i = 1, im
        if(cnvflg(i) .and. aa1(i) <= 0.0_conv_wp) then
           cnvflg(i) = .false.
        endif
      enddo
!!
      ...
      do k = 2, km1
        do i = 1, im
          if (cnvflg(i) .and. k < ktcon(i)) then
              aup = 1.0_conv_wp
              if(k <= kb(i)) aup = 0.0_conv_wp
              adw = 1.0_conv_wp
              if(k > jmin(i)) adw = 0.0_conv_wp
              dp = 1000.0_conv_wp * del(i,k)
              dz = zi(i,k) - zi(i,k-1)
c
              dv1h = heo(i,k)
...
cj
     ...
     &                      etad(i,k))*dv1h-(aup*eta(i,k-1)-adw*edto(i)*
     &                      etad(i,k-1))*dv3h-(aup*tem*eta(i,k-1)+adw*
     &                      edto(i)*ptem*etad(i,k))*dv2h*dz+aup*tem1*
     &                      eta(i,k-1)*.5_conv_wp*(hcko(i,k)+
     &                      hcko(i,k-1))*dz+adw*edto(i)*ptem1*etad(i,k)*
     &                      .5_conv_wp*(hcdo(i,k)+hcdo(i,k-1))*dz)* 
     &                      factor
cj
...
cj
              ...
              dellae(i,k,n) = dellae(i,k,n) + (aup*(tem1-tem2)-adw*
     &                      edto(i)*(ptem1-ptem2))*real(grav,conv_wp)/dp
cj
          endif
        enddo
      enddo
...
c
      do i = 1, im
        if(cnvflg(i)) then
          ...
          tem = eta(i,indx-1) * real(grav,conv_wp) / dp
...
c
     ...
     &             ecko(i,indx-1,n) * real(grav, conv_wp) / dp
        endif
      enddo
      enddo
      endif
...
!
      do k=1,km1
        do i=1,im
          if(cnvflg(i) .and. k <= ktcon(i)) then
            q_diff(i,k) = real(q1(i,k), conv_wp) - real(q1(i,k+1),
     &                    conv_wp)
          endif
        enddo
      enddo
      do i=1,im
        if(cnvflg(i)) then
          if(real(q1(i,1), conv_wp) >= 0.0_conv_wp) then
      ...
      do k = 1, km1
        do i = 1, im
          if(cnvflg(i) .and. k < ktcon(i)) then
            ...
            if(tem > 0.0_conv_wp) then
              rrkp = 0.0_conv_wp
              if(abs(q_diff(i,k)) > 1.e-22_conv_wp)
     &               rrkp = q_diff(i,k+1) / q_diff(i,k)
              phkp = (rrkp+abs(rrkp)) / (1.0_conv_wp+abs(rrkp))
              tem1 = real(q1(i,k+1), conv_wp) +
     &                   phkp*(qo(i,k)-real(q1(i,k+1), conv_wp))
              flxtvd(i,k) = tem * tem1
            elseif(tem < 0.0_conv_wp) then
              rrkp = 0.0_conv_wp
              if(abs(q_diff(i,k)) > 1.e-22_conv_wp)
     &               rrkp = q_diff(i,k-1) / q_diff(i,k)
              phkp = (rrkp+abs(rrkp)) / (1.0_conv_wp+abs(rrkp))
              tem1 = real(q1(i,k), conv_wp) +
     &                  phkp*(qo(i,k)-real(q1(i,k), conv_wp))
              flxtvd(i,k) = tem * tem1
            else
              tem1 = qo(i,k)
        ...
        do i = 1, im
          if (asqecflg(i) .and. k <= kmax(i)) then
            ...
            if(k <= ktcon(i)) then
              ...
              dellat = (dellah(i,k) - real(hvap, conv_wp) * dellaq(i,k))
     &                 / real(cp, conv_wp)           
...
!! - Recalculate moist static energy and saturation moist static energy.
     ...
     &           (real(cp, conv_wp) * (1.0_conv_wp + gamma))
            dq = dqsdt * dt + dqsdp * dp
            to(i,k) = to(i,k+1) + dt
            qo(i,k) = qo(i,k+1) + dq
            po(i,k) = .5_conv_wp * (pfld(i,k) + pfld(i,k+1))
...
!           qo(i,k)   = min(qo(i,k),qeso(i,k))
     ...
     &                  real(hvap, conv_wp) * qeso(i,k)
      ...
      do i = 1, im
        if(asqecflg(i)) then
          k = kmax(i)
          heo(i,k) = real(grav, conv_wp) * zo(i,k) + real(cp, conv_wp) *
     &               to(i,k) + real(hvap, conv_wp) * qo(i,k)
          heso(i,k) = real(grav, conv_wp) * zo(i,k) + real(cp, conv_wp)*
     &                to(i,k) + real(hvap, conv_wp) * qeso(i,k)
c         heo(i,k) = min(heo(i,k),heso(i,k))
        endif
      enddo
c
        ...
        do i = 1, im
          if (asqecflg(i)) then
            if(k > kb(i) .and. k <= ktcon(i)) then
              ...
              factor = 1.0_conv_wp + tem - tem1
              hcko(i,k) = ((1.0_conv_wp-tem1)*hcko(i,k-1)+tem*
     &                    0.50_conv_wp*(heo(i,k)+heo(i,k-1)))/factor
            endif
          endif
        enddo
      enddo
        ...
        do i = 1, im
          if (asqecflg(i)) then
            if(k > kb(i) .and. k < ktcon(i)) then
              ...
              xqrch = qeso(i,k) + gamma * xdby / (real(hvap, conv_wp) *
     &                (1.0_conv_wp + gamma))
cj
              tem  = 0.25_conv_wp * (xlamue(i,k)+xlamue(i,k-1)) * dz
              tem  = real(cq, conv_wp) * tem
...
!                 xaa0(i) = xaa0(i) - dz * grav * qlk * etah
                  xaa0(i) = xaa0(i) - dz * real(grav, conv_wp) * qlk
                endif
                qcko(i,k) = qlk + xqrch
      ...
      do k = km1, 1, -1
        do i = 1, im
          if (asqecflg(i) .and. k < jmin(i)) then
              dq = qeso(i,k)
              dt = to(i,k)
              gamma    = real(el2orc, conv_wp) * dq / dt**2
              dh       = hcdo(i,k) - heso(i,k)
              qrcd(i,k)= dq + (1.0_conv_wp / real(hvap, conv_wp)) *
     &                   (gamma / (1.0_conv_wp+gamma)) * dh
!             detad    = etad(i,k+1) - etad(i,k)
cj
              dz = zi(i,k+1) - zi(i,k)
              tem  = 0.5_conv_wp * xlamdet(i) * dz
...
!             xaa0(i)=xaa0(i)+edtx(i)*dz*etad(i,k)
              xaa0(i) = xaa0(i)+edtx(i)*dz*(real(grav, conv_wp)/
     &                 (real(cp, conv_wp)*dt))*((dhh-dh)/(1.0_conv_wp+
     &                 dg))*(1.0_conv_wp+real(fv, conv_wp)*real(cp,
     &                 conv_wp)*dg*dt/real(hvap, conv_wp))
              val = 0.0_conv_wp
!             xaa0(i)=xaa0(i)+edtx(i)*dz*etad(i,k)
              xaa0(i) = xaa0(i)+edtx(i)*dz*real(grav, conv_wp)*real(fv,
     &                  conv_wp)*max(val,(qeso(i,k)-qo(i,k)))
          endif
        enddo
      enddo
c
        ...
        do i = 1, im
          if(cnvflg(i)) then
            if(k >= kbcon1(i) .and. k < ktcon1(i)) then
              dz = zi(i,k) - zi(i,k-1)
              tem = sqrt(real(u1(i,k), conv_wp)*real(u1(i,k),conv_wp)+
     &              real(v1(i,k), conv_wp)*real(v1(i,k), conv_wp))
...
!Initial computations, dynamic q-tendency                                                                                                                                               
     ...
     &           .or. sigmab_coldstart))then
            ...
            do k = 1,km
               do i = 1,im
                  qadv(i,k)=(real(q(i,k), conv_wp) -real(prevsq(i,k),
     &                      conv_wp))*invdelt
               enddo
            enddo
         endif
         ...
         do k = 1,km
            do i = 1,im
               tmfq(i,k)=real(tmf(i,k,1), conv_wp)
            enddo
         enddo
...
!!  As discussed in Han et al. (2017) \cite han_et_al_2017 , when dtconv is larger than tauadv, the convective mixing is not fully conducted before the cumulus cloud is advected out of the grid cell. In this case, therefore, the cloud base mass flux is further reduced in proportion to the ratio of tauadv to dtconv.
      ...
      do i= 1, im
        if(cnvflg(i) .and. .not.asqecflg(i)) then
          k = kbcon(i)
          rho = po(i,k)*100.0_conv_wp / (real(rd, conv_wp)*to(i,k))
          if(progsigma)then
...
!> - For scale-aware parameterization, the updraft fraction (sigmagfm) is first computed as a function of the lateral entrainment rate at cloud base (see Han et al.'s (2017) \cite han_et_al_2017 equation 4 and 5), following the study by Grell and Freitas (2014) \cite grell_and_freitas_2014.
      do i = 1, im
        if(cnvflg(i)) then
          tem = min(max(xlamue(i,kbcon(i)), 7.e-5_conv_wp), 
     &          3.e-4_conv_wp)
          tem = 0.2_conv_wp / tem
          tem1 = 3.14_conv_wp * tem * tem
          sigmagfm(i) = tem1 / real(garea(i), conv_wp)
...
c
     ...
     &                  real(epsm1, conv_wp) *qeso(i,k))
      ...
      if (.not.hwrf_samfdeep) then
        ...
        do i = 1, im
          if (cnvflg(i) .and. k <= kmax(i)) then
            ctro(i,k,n) = real(qtr(i,k,kk), conv_wp)
          endif
        enddo
        enddo
        enddo
        ...
        do i = 1, im
          if (cnvflg(i) .and. k <= kmax(i)) then
            if(k <= ktcon(i)) then
              ...
              dellat = (dellah(i,k) - real(hvap, conv_wp) *dellaq(i,k))/
     &                 real(cp, conv_wp)
              t1(i,k) = real(t1(i,k), kind_phys) +
     &                  real(tem2 * dellat, kind_phys)
              q1(i,k) = real(q1(i,k), kind_phys) +
     &                  real(tem2 * dellaq(i,k), kind_phys)
!             tem = tem2 / rcs(i)
!             u1(i,k) = u1(i,k) + dellau(i,k) * tem
!             v1(i,k) = v1(i,k) + dellav(i,k) * tem
              u1(i,k) = real(u1(i,k), kind_phys) +
     &                  real(tem2 * dellau(i,k), kind_phys)
              v1(i,k) = real(v1(i,k), kind_phys) +
     &                  real(tem2 * dellav(i,k), kind_phys)
              dp = 1000.0_conv_wp* del(i,k)
              tem = xmb(i) * dp / real(grav, conv_wp)
...
!
      ...
      do k = 1,km1
        do i = 1,im
          if(cnvflg(i) .and. k <= ktcon(i)) then
            tem = real(q1(i,k),conv_wp)*real(delp(i,k),conv_wp)/
     &            real(grav, conv_wp)
            if(real(q1(i,k), conv_wp) < 0.0_conv_wp) tsumn(i)= tsumn(i)+
     &      tem
            if(real(q1(i,k), conv_wp) > 0.0_conv_wp) tsump(i)= tsump(i)+
     &      tem
          endif
        enddo
      enddo
        ...
        do i = 1,im
          if(cnvflg(i) .and. k <= ktcon(i)) then
            if(rtnp(i) < 0.0_conv_wp) then
              if(tsump(i) > abs(tsumn(i))) then
                if(real(q1(i,k), conv_wp) < 0.0_conv_wp) q1(i,k) =
     &            0.0_conv_wp
                if(real(q1(i,k), conv_wp) > 0.0_conv_wp) q1(i,k) =
     &            (1.0_conv_wp+rtnp(i))*real(q1(i,k), conv_wp)
              else
                if(real(q1(i,k), conv_wp) < 0.0_conv_wp) q1(i,k) =
     &            (1.0_conv_wp+rtnp(i))*real(q1(i,k), conv_wp)
                if(real(q1(i,k), conv_wp) > 0.0_conv_wp) q1(i,k) =
          ...
          do i = 1,im
            if(cnvflg(i) .and. k <= ktcon(i)) then
              if(n == indx) then
                ...
                tem = ctr(i,k,n) * real(delp(i,k), conv_wp) /
     &                real(grav, conv_wp)
            ...
            if(cnvflg(i) .and. k <= ktcon(i)) then
              if(rtnp(i) < 0.0_conv_wp) then
                if(tsump(i) > abs(tsumn(i))) then
                  ...
                  if(ctr(i,k,n)>0.0_conv_wp) ctr(i,k,n)=(1.0_conv_wp+
     &              rtnp(i))*ctr(i,k,n)
                else
                  if(ctr(i,k,n)<0.0_conv_wp) ctr(i,k,n)=(1.0_conv_wp+
     &              rtnp(i))*ctr(i,k,n)
...
!
        ...
        do i = 1, im
          if(cnvflg(i) .and. k <= ktcon(i)) then
            qtr(i,k,kk) = real(ctr(i,k,n), kind_phys)
          endif
        enddo
        enddo
!
          ...
          do k = 2, km1
            do i = 1, im
              if (cnvflg(i)) then
                if(k > kb(i) .and. k < ktcon(i)) then
                  dp = 1000.0_conv_wp * del(i,k)
                  wet_dep(i,k,n) = chem_pw(i,k,n)*real(grav, conv_wp)/dp
...
!   borrow negative mass from wet deposition
                    ...
                    if(wet_dep(i,k,n) >= tem) then
                      ...
                      qtr(i,k,kk) = 0.0_conv_wp
                    else
                      wet_dep(i,k,n) = 0.0_conv_wp
                      qtr(i,k,kk) = real(qtr(i,k,kk), kind_phys)+
     &                              real(wet_dep(i,k,n)/dp, kind_phys)
                    endif
                  endif
                endif
              endif
...
!             if(islimsk(i) == 1) evef=.07
              qcond(i) = real(evef, conv_wp) * (real(q1(i,k), conv_wp) -
     &            qeso(i,k))/ (1.0_conv_wp + real(el2orc, conv_wp) *
     &            qeso(i,k) / real(t1(i,k), conv_wp)**2)
              dp = 1000.0_conv_wp * del(i,k)
              tem = real(grav, conv_wp) / dp
              tem1 = dp / real(grav, conv_wp)
              if(rn(i) > 0.0_conv_wp .and. qcond(i) < 0.0_conv_wp) then
                qevap(i) = -qcond(i) * (1.0_conv_wp-exp(-.32_conv_wp*
     &                     sqrt(dt2*rn(i))))
                qevap(i) = min(qevap(i), rn(i)*1000.0_conv_wp*tem)
                delq2(i) = delqev(i) + .001_conv_wp * qevap(i) * tem1
              endif
              if(rn(i) > 0.0_conv_wp .and. qcond(i) < 0.0_conv_wp .and.
     &           delq2(i) > rntot(i)) then
                qevap(i) = 1000.0_conv_wp* tem * (rntot(i) - delqev(i))
                flg(i) = .false.
              endif
              if(rn(i) > 0.0_conv_wp .and. qevap(i) > 0.0_conv_wp) then
                q1(i,k) = real(q1(i,k), kind_phys) + real(qevap(i),
     &                    kind_phys)
                t1(i,k) = real(t1(i,k), kind_phys) -
     &                    real(elocp * qevap(i), kind_phys)
...
c
          if(rn(i) < 0.0_conv_wp .and. .not.flg(i)) rn(i) = 0.0_conv_wp
          if(rn(i) <= 0.0_conv_wp) then
            rn(i) = 0.0_conv_wp
          else
            ktop(i) = ktcon(i)
            kbot(i) = kbcon(i)
...
!> - Calculate convective cloud water.
      do k = 1, km
         do i = 1, im
            if (cnvflg(i) .and. rn(i) > 0.0_conv_wp) then
               if (k >= kbcon(i) .and. k < ktcon(i)) then
...
!> - Calculate convective cloud cover, which is used when pdf-based cloud fraction is used (i.e., pdfcld=.true.).
      do k = 1, km
        do i = 1, im
          if (cnvflg(i) .and. rn(i) > 0.0_conv_wp) then
            if (k >= kbcon(i) .and. k < ktcon(i)) then
              cnvc(i,k) = 0.04_conv_wp * log(1.0_conv_wp + 
...
!> - If convective precipitation is zero or negative, reset the updated state variables back to their original values (negating convective changes).
      do k = 1, km
        do i = 1, im
          if(cnvflg(i) .and. rn(i) <= 0.0_conv_wp) then
            if (k <= kmax(i)) then
              t1(i,k) = real(to(i,k), kind_phys)
              q1(i,k) = real(qo(i,k), kind_phys)
              u1(i,k) = real(uo(i,k), kind_phys)
              v1(i,k) = real(vo(i,k), kind_phys)
            endif
          endif
        enddo
      enddo
      ...
      do k = 1, km
        do i = 1, im
          if(cnvflg(i) .and. rn(i) <= 0.0_conv_wp) then
            if (k <= kmax(i)) then
              qtr(i,k,kk)= real(ctro(i,k,n), kind_phys)
            endif
          endif
        enddo
      enddo
        ...
        do i = 1, im
          if(cnvflg(i) .and. rn(i) > 0.0_conv_wp) then
            if(k > kb(i) .and. k < ktop(i)) then
              ...
              tem1 = pfld(i,k) * 100.0_conv_wp /
     &                   (real(rd, conv_wp) * real(t1(i,k), conv_wp))
              if(progsigma)then
                tem2 = sigmab(i)
              else
      ...
      if(mp_phys == mp_phys_mg) then
        do k=1,km
          do i=1,im
            ...
            cf_upi(i,k)   = cnvc(i,k)
            w_upi(i,k)    = ud_mf(i,k)*real(t1(i,k), conv_wp)*real(rd,
     &                      conv_wp)/(dt2*max(sigmagfm(i),
     &                      1.e-12_conv_wp)*real(prslp(i,k), conv_wp))
